...
router.post('/generate', async function (request, response) {
    try {
        ...
        if (request.body.chat_completion_source === CHAT_COMPLETION_SOURCES.OPENAI) {
            ...
            if (isClaude) {
                // Strip parameters not supported by Anthropic's API.
                // OpenRouter may not filter these for newer models like Opus 4.6.
                delete request.body.presence_penalty;
                delete request.body.frequency_penalty;
                delete request.body.logit_bias;
                delete request.body.seed;
                delete request.body.n;
            }
        ...
        if (fetchResponse.ok) {
            ...
            if (!response.headersSent) {
                const errorObj = errorData?.error || { message };
                response.send({ error: errorObj, quota_error: quota_error });
