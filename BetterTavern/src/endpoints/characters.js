...
const processCharacter = async (item, directories, { shallow }) => {
    try {
        ...
        // If the chat property doesn't match an existing file, use the first existing chat
        if (fs.existsSync(chatsDirectory) && jsonObject.chat) {
            const chatFilePath = path.join(chatsDirectory, `${jsonObject.chat}.jsonl`);
            if (!fs.existsSync(chatFilePath)) {
                const existingChats = fs.readdirSync(chatsDirectory).filter(f => f.endsWith('.jsonl'));
                if (existingChats.length > 0) {
                    jsonObject.chat = path.parse(existingChats[0]).name;
                }
            }
        }
...
router.post('/duplicate', validateAvatarUrlMiddleware, async function (request, response) {
    try {
        ...
        const rawOldData = await readCharacterData(filename);
        if (!rawOldData) {
            console.error('Failed to read character data for duplicate', filename);
            return response.sendStatus(500);
        }
        const oldData = getCharaCardV2(JSON.parse(rawOldData), request.user.directories);
        const originalName = oldData?.name || oldData?.data?.name || baseName;
        const baseDisplayName = String(originalName).replace(/\s*\(\d+\)\s*$/, '').trim();
        const displayName = `${baseDisplayName} (${suffix})`;
        _.set(oldData, 'data.name', displayName);
        _.set(oldData, 'name', displayName);
        delete oldData.chat;
        const newData = JSON.stringify(oldData);
        const newInternalName = path.parse(newFilename).name;
        await writeCharacterData(filename, newData, newInternalName, request);
