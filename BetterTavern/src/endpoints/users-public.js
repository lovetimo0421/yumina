...
import { KEY_PREFIX, getUserAvatar, toKey, getPasswordHash, getPasswordSalt, lazyInitUser, getUserInitStatus } from '../users.js';
...
router.post('/login', async (request, response) => {
    try {
        ...
        // Check if user needs lazy initialization (first login)
        const initStatus = getUserInitStatus(user.handle);
        if (!initStatus.ready && !initStatus.initializing) {
            // Start async initialization - don't wait for it
            lazyInitUser(user.handle);
            return response.json({ handle: user.handle, initializing: true });
...
router.get('/init-status', async (request, response) => {
    try {
        ...
        const status = getUserInitStatus(handle);
