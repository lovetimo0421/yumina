...
/**
 * Mapping from SECRET_KEYS to environment variable names for server-wide fallback.
 * When a user doesn't have a key, the system will check these environment variables.
 * This enables "free API" mode where the server provides a shared key for all users.
 */
export const SECRET_ENV_FALLBACK = {
    [SECRET_KEYS.OPENAI]: 'ST_OPENAI_API_KEY',
    [SECRET_KEYS.CLAUDE]: 'ST_CLAUDE_API_KEY',
    [SECRET_KEYS.OPENROUTER]: 'ST_OPENROUTER_API_KEY',
    [SECRET_KEYS.MAKERSUITE]: 'ST_GOOGLE_API_KEY',
    [SECRET_KEYS.MISTRALAI]: 'ST_MISTRAL_API_KEY',
    [SECRET_KEYS.GROQ]: 'ST_GROQ_API_KEY',
    [SECRET_KEYS.COHERE]: 'ST_COHERE_API_KEY',
    [SECRET_KEYS.PERPLEXITY]: 'ST_PERPLEXITY_API_KEY',
    [SECRET_KEYS.DEEPSEEK]: 'ST_DEEPSEEK_API_KEY',
    [SECRET_KEYS.XAI]: 'ST_XAI_API_KEY',
    [SECRET_KEYS.TOGETHERAI]: 'ST_TOGETHERAI_API_KEY',
    [SECRET_KEYS.FIREWORKS]: 'ST_FIREWORKS_API_KEY',
    [SECRET_KEYS.AI21]: 'ST_AI21_API_KEY',
};
...
export class SecretManager {
     ...
     * Falls back to environment variable if user doesn't have the key configured.
     * @param {string} key Secret key
     * @param {string?} id ID of the secret to read (optional)
     * @returns {string} Secret value or empty string if not found
     */
    readSecret(key, id) {
        // First, try to read from user's secrets file
        if (fs.existsSync(this.filePath)) {
            const secrets = this._readSecretsFile();
            const secretArray = secrets[key];
        ...
        // Fallback: Check environment variable for server-wide shared key
        const envVarName = SECRET_ENV_FALLBACK[key];
    ...
    getSecretState() {
        ...
        for (const key of Object.values(SECRET_KEYS)) {
            ...
            if (value && Array.isArray(value) && value.length > 0) {
                ...
                // Check if a server-wide fallback key is available
                const envVarName = SECRET_ENV_FALLBACK[key];
                if (envVarName && process.env[envVarName]) {
                    // Return a special entry indicating server-provided key
                    state[key] = [{
                        id: 'server-provided',
                        value: this.getMaskedValue(process.env[envVarName], key),
