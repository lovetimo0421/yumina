...
import { getConfigValue, color, setPermissionsSync, isValidUrl, humanizedDateTime } from '../util.js';
...
const INIT_CHARACTERS_LOG_KEY = '__default_characters_initialized_v2__';
const INIT_MVU_SETTINGS_LOG_KEY = '__mvu_notifications_disabled__';
...
export async function checkForNewContent(directoriesList, forceCategories = []) {
    try {
        ...
        for (const directories of directoriesList) {
            ...
            await initializeDefaultCharacters(directories);
            disableMvuNotifications(directories);
        }
...
/**
 * Initializes content for a single user (lazy init on first login).
 * Seeds default content, initializes characters, and disables MVU notifications.
 * @param {import('../users.js').UserDirectoryList} directories User directories
 * @returns {Promise<void>}
 */
export async function initializeUserContent(directories) {
    try {
        const contentIndex = getContentIndex();
        ...
        console.error('User content initialization failed', err);
    }
}
...
/**
 * Converts a V2 character_book to SillyTavern world info format.
 * Server-side port of the client's convertCharacterBook() from world-info.js.
 * @param {object} characterBook Character book object from card data
 * @returns {object} World info object with entries and originalData
 */
function convertCharacterBookToWorldInfo(characterBook) {
    ...
    characterBook.entries.forEach((entry, index) => {
        ...
        result.entries[entry.id] = {
            ...
            position: entry.extensions?.position ?? (entry.position === 'before_char' ? POSITION_BEFORE : POSITION_AFTER),
            ...
            selectiveLogic: entry.extensions?.selectiveLogic ?? WI_LOGIC_AND_ANY,
            ...
            role: entry.extensions?.role ?? ROLE_SYSTEM,
...
/**
 * Creates a greeting chat JSONL file for a default character.
 * Skips if any .jsonl files already exist in the chat directory.
 * @param {string} chatDir Path to the character's chat directory
 * @param {string} charName Character name
 * @param {string} firstMes First message / greeting text
 * @param {string} pngFileName PNG filename (used for logging)
 * @returns {string|null} Chat name (without .jsonl) if created, or existing chat name, or null
 */
function createGreetingChat(chatDir, charName, firstMes, pngFileName, alternateGreetings) {
    if (!fs.existsSync(chatDir)) {
        fs.mkdirSync(chatDir, { recursive: true });
    }

    // Check if any .jsonl files already exist
    const existingChats = fs.readdirSync(chatDir).filter(f => f.endsWith('.jsonl'));
    if (existingChats.length > 0) {
        console.info(`[Default chars] Chat already exists for ${pngFileName}, skipping`);
        // Return the existing chat name so the character can still be linked to it
        return path.parse(existingChats[0]).name;
    }

    const timestamp = humanizedDateTime();
    const chatName = sanitize(`${charName} - ${timestamp}`);
    ...
    if (Array.isArray(alternateGreetings) && alternateGreetings.length > 0) {
        greeting.swipe_id = 0;
        greeting.swipes = [firstMes, ...alternateGreetings];
        greeting.swipe_info = greeting.swipes.map(() => ({
            send_date: timestamp,
    ...
    fs.writeFileSync(chatFilePath, `${metadata}\n${JSON.stringify(greeting)}`);
    setPermissionsSync(chatFilePath);
    console.info(`[Default chars] Created greeting chat for ${pngFileName}`);
    return chatName;
}

/**
 * Initializes default characters by creating greeting chats and extracting embedded world books.
 * Runs once per user, tracked by content.log.
 * @param {import('../users.js').UserDirectoryList} directories User directories
 */
async function initializeDefaultCharacters(directories) {
    const contentLogPath = path.join(directories.root, 'content.log');
    const contentLog = getContentLog(contentLogPath);

    if (contentLog.includes(INIT_CHARACTERS_LOG_KEY)) {
        return;
    }

    const contentIndex = getContentIndex();
    const characterItems = contentIndex.filter(item => item.type === CONTENT_TYPES.CHARACTER);

    for (const charItem of characterItems) {
        const pngFileName = path.parse(charItem.filename).base;
        const pngPath = path.join(directories.characters, pngFileName);

        if (!fs.existsSync(pngPath)) {
            console.warn(`[Default chars] Character PNG not found: ${pngPath}`);
            continue;
        }

        try {
            // Read character metadata from PNG
            const pngBuffer = fs.readFileSync(pngPath);
            const rawData = read(pngBuffer);
            const charData = JSON.parse(rawData);
            const data = charData.data || charData;
            const charName = data.name || path.parse(pngFileName).name;
            let needsPngUpdate = false;

            // 1. Create greeting chat if first_mes exists and set chat property
            const firstMes = data.first_mes;
            if (firstMes) {
                const chatDirName = path.parse(pngFileName).name;
                const chatDir = path.join(directories.chats, chatDirName);
                const chatName = createGreetingChat(chatDir, charName, firstMes, pngFileName, data.alternate_greetings);

                // Set the chat property so the character opens to this chat
                if (chatName && charData.chat !== chatName) {
                    charData.chat = chatName;
                    needsPngUpdate = true;
                }
            } else {
                console.info(`[Default chars] No first_mes for ${pngFileName}, skipping chat creation`);
            }

            // 2. Extract embedded world book if present
            ...
            if (characterBook && Array.isArray(characterBook.entries) && characterBook.entries.length > 0) {
                const existingWorld = data.extensions?.world;
                const bookName = characterBook.name || `${charName} Lore`;

                // Skip world extraction if already linked and exists on disk
                let skipWorldExtraction = false;
                if (existingWorld) {
                    const existingWorldPath = path.join(directories.worlds, `${existingWorld}.json`);
                    if (fs.existsSync(existingWorldPath)) {
                        console.info(`[Default chars] World "${existingWorld}" already linked for ${pngFileName}, skipping extraction`);
                        skipWorldExtraction = true;
                    }
                }

                if (!skipWorldExtraction) {
                    const worldFilePath = path.join(directories.worlds, `${bookName}.json`);

                    // Don't overwrite an existing world file
                    if (fs.existsSync(worldFilePath)) {
                        console.info(`[Default chars] World file "${bookName}.json" already exists, skipping extraction`);
                    } else {
                        const worldInfo = convertCharacterBookToWorldInfo(characterBook);
                        fs.writeFileSync(worldFilePath, JSON.stringify(worldInfo, null, 4));
                        setPermissionsSync(worldFilePath);
                        console.info(`[Default chars] Extracted world book "${bookName}" from ${pngFileName}`);
                    }

                    // Update card metadata to link the world
                    if (!existingWorld || existingWorld !== bookName) {
                        if (!data.extensions) {
                            data.extensions = {};
                        }
                        data.extensions.world = bookName;
                        needsPngUpdate = true;
                    }
                }
            }

            // 3. Write updated PNG if any metadata changed (chat name or world link)
            if (needsPngUpdate) {
                const updatedCard = charData.data ? charData : { data: data };
                const updatedBuffer = write(pngBuffer, JSON.stringify(updatedCard));
                fs.writeFileSync(pngPath, updatedBuffer);
                setPermissionsSync(pngPath);
                console.info(`[Default chars] Updated ${pngFileName} metadata`);
            }
        } catch (error) {
            console.error(`[Default chars] Failed to process ${pngFileName}:`, error);
            continue;
        }
    }

    contentLog.push(INIT_CHARACTERS_LOG_KEY);
    writeFileAtomicSync(contentLogPath, contentLog.join('\n'));
    console.info('[Default chars] Default character initialization complete');
}

/**
 * Injects MVU notification settings (all disabled) into user's settings.json.
 * Runs once per user, tracked by content.log.
 * @param {import('../users.js').UserDirectoryList} directories User directories
 */
function disableMvuNotifications(directories) {
    const contentLogPath = path.join(directories.root, 'content.log');
    const contentLog = getContentLog(contentLogPath);

    if (contentLog.includes(INIT_MVU_SETTINGS_LOG_KEY)) {
        return;
    }
    ...
    try {
        ...
        if (fs.existsSync(settingsPath)) {
            ...
            if (settings.extension_settings && !settings.extension_settings.mvu_settings) {
                settings.extension_settings.mvu_settings = {
                    '通知': {
                        'MVU框架加载成功': false,
                        '变量初始化成功': false,
                        '变量更新出错': false,
                ...
                console.info('[Default chars] Injected MVU notification settings (disabled)');
            }
        }
    } catch (error) {
        console.error('[Default chars] Failed to inject MVU settings:', error);
    }

    contentLog.push(INIT_MVU_SETTINGS_LOG_KEY);
    writeFileAtomicSync(contentLogPath, contentLog.join('\n'));
}
