...
import { getContentOfType, initializeUserContent } from './endpoints/content-manager.js';
...
export async function ensurePublicDirectoriesExist() {
    ...
    // Only create directories for users whose root folder already exists (lazy init)
    const userHandles = await getAllUserHandles();
    const directoriesList = [];
    for (const handle of userHandles) {
        const directories = getUserDirectories(handle);
        if (!fs.existsSync(directories.root)) {
            continue; // Skip users who haven't logged in yet
        }
        directoriesList.push(directories);
...
export async function getUserDirectoriesList() {
    ...
    for (const handle of userHandles) {
        const directories = getUserDirectories(handle);
        // Only include users whose root folder exists (lazy init: skip uninitialized users)
        if (fs.existsSync(directories.root)) {
            directoriesList.push(directories);
        }
    }
...
/** @type {Map<string, Promise<void>>} Tracks in-progress lazy init promises by user handle */
const LAZY_INIT_IN_PROGRESS = new Map();
/** @type {Set<string>} Tracks completed lazy inits */
const LAZY_INIT_COMPLETED = new Set();

/**
 * Lazily initializes a user's data directory on first login.
 * Creates directories, seeds content, generates thumbnails.
 * Safe to call multiple times - will only run once per user.
 * @param {string} handle User handle
 * @returns {Promise<void>}
 */
export async function lazyInitUser(handle) {
    const directories = getUserDirectories(handle);

    // Already initialized (.initialized marker exists = full init complete)
    const readyMarkerPath = path.join(directories.root, '.initialized');
    if (fs.existsSync(readyMarkerPath)) {
        LAZY_INIT_COMPLETED.add(handle);
        return;
    }

    // Already in progress - wait for existing init
    if (LAZY_INIT_IN_PROGRESS.has(handle)) {
        return LAZY_INIT_IN_PROGRESS.get(handle);
    }

    const initPromise = (async () => {
        try {
            console.info(`[Lazy Init] Initializing data for user: ${handle}`);
            ...
            // Seed default content (characters, backgrounds, themes, etc.)
            await initializeUserContent(directories);

            // Generate thumbnail cache for backgrounds
            await ensureThumbnailCache([directories]);

            // Pre-configure settings.json for seamless first login
            ...
            if (fs.existsSync(settingsPath)) {
                try {
                    ...
                    if (userRecord?.name) {
                        settings.username = userRecord.name;
                    }
                    settings.firstRun = false;      // Don't block init with onboarding popup
                    fs.writeFileSync(settingsPath, JSON.stringify(settings, null, 4));
                } catch { /* non-critical */ }
            }

            // Write .initialized marker (MUST be last - signals full readiness)
            fs.writeFileSync(path.join(directories.root, '.initialized'), '');

            LAZY_INIT_COMPLETED.add(handle);
            console.info(`[Lazy Init] Completed initialization for user: ${handle}`);
        } catch (err) {
            console.error(`[Lazy Init] Failed to initialize user: ${handle}`, err);
            throw err;
        } finally {
            LAZY_INIT_IN_PROGRESS.delete(handle);
        }
    })();

    LAZY_INIT_IN_PROGRESS.set(handle, initPromise);
    return initPromise;
}

/**
 * Checks if a user's lazy initialization is complete.
 * @param {string} handle User handle
 * @returns {{ ready: boolean, initializing: boolean }}
 */
export function getUserInitStatus(handle) {
    if (LAZY_INIT_COMPLETED.has(handle)) {
        return { ready: true, initializing: false };
    }
 ...
 */
/**
 * Predefined users to ensure exist on startup.
 * Each entry: handle is used as both username and password.
 */
...
export async function initUserStorage(dataRoot) {
    ...
    for (const handle of PREDEFINED_USERS) {
        if (keys.includes(handle)) continue;
        const salt = getPasswordSalt();
        const newUser = {
            handle: handle,
            name: handle,
            created: Date.now(),
            password: getPasswordHash(handle, salt),
            salt: salt,
            admin: false,
            enabled: true,
        };
        await storage.setItem(toKey(handle), newUser);
        console.info(`[Predefined Users] Created user: ${handle}`);
    }
}
...
export async function setUserDataMiddleware(request, response, next) {
    ...
    // Safety net: block requests until user is fully initialized
    const readyMarkerPath = path.join(directories.root, '.initialized');
    if (!fs.existsSync(readyMarkerPath)) {
        // Don't block the init-status endpoint (polling needs it)
        if (request.path === '/api/users/init-status') {
            request.user = { profile: user, directories: directories };
            return next();
        }
        // Block until init completes
        if (LAZY_INIT_IN_PROGRESS.has(handle)) {
            await LAZY_INIT_IN_PROGRESS.get(handle);
        } else {
            await lazyInitUser(handle);
        }
    }
