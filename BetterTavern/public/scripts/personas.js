...
/**
 * Updates the current user avatar without triggering persona selection side-effects.
 * Intended for settings-driven avatar changes.
 * @param {string} avatarId
 * @param {object} [options]
 * @param {boolean} [options.forceReload=false]
 */
export function setUserAvatarFromSettings(avatarId, { forceReload = false } = {}) {
    const nextAvatar = avatarId || '';
...
/**
 * Synchronizes user messages in the current chat to the latest name/avatar.
 * @param {object} [options]
 * @param {boolean} [options.updateAvatar=false]
 * @param {boolean} [options.save=true]
 */
export async function syncUserMessagesToCurrentIdentity({ updateAvatar = false, save = true } = {}) {
    if (!Array.isArray(chat) || chat.length === 0) {
        return;
    }

    const avatarUrl = updateAvatar ? getThumbnailUrl('persona', user_avatar) : null;

    for (const mes of chat) {
        if (mes.is_user) {
            mes.name = name1;
            if (updateAvatar && avatarUrl) {
                mes.force_avatar = avatarUrl;
            }
        }
    }

    const userMessages = $('.mes[is_user="true"]');
    userMessages.attr('ch_name', name1);
    userMessages.find('.name_text').text(name1);

    if (updateAvatar && avatarUrl) {
        userMessages.attr('force_avatar', true);
        userMessages.find('.avatar img').attr('src', avatarUrl);
    }
