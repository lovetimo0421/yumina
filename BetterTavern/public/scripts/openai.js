...
const default_settings = {
    preset_settings_openai: '⚓YuminaDefault',
    temp_openai: 1.0,
    freq_pen_openai: 0,
    pres_pen_openai: 0,
    top_p_openai: 1.0,
...
function setupChatCompletionPromptManager(openAiSettings) {
    ...
    if (promptManager) {
        // If a character is loaded, trigger a full render with dry-run to populate token counts
...
export function tryParseStreamingError(response, decoded, { quiet = false } = {}) {
    try {
        ...
        if (data.error) {
            let errorMessage = data.error.message || response.statusText;
            if (data.error.metadata?.raw) {
                ...
                } catch { /* raw isn't JSON, use as-is */ }
            }
            !quiet && toastr.error(errorMessage, 'Chat Completion API');
...
async function sendOpenAIRequest(type, messages, signal, { jsonSchema = null } = {}) {
    ...
    if (stream) {
        ...
        if (data.error) {
            let message = data.error.message || response.statusText || t`Unknown error`;
            if (data.error.metadata?.raw) {
                try {
                    const raw = JSON.parse(data.error.metadata.raw);
                    if (raw?.error?.message) {
