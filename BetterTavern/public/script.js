...
import { showWorldLoader, finishWorldLoader } from './scripts/world-loader.js';
...
import { ensurePermanentAssistant, initWelcomeScreen, openPermanentAssistantChat, openPermanentAssistantCard, getPermanentAssistantAvatar } from './scripts/welcome-screen.js';
import { showCharacterProfile } from './scripts/bettertavern-character-profile.js';
...
export const systemUserName = 'Yumina System';
export const neutralCharacterName = 'Yumina Assistant';
...
async function firstLoadInit() {
    ...
    const isInitializing = !!localStorage.getItem('initializing');
    if (isInitializing) {
    ...
    initBulkEdit();
    initReasoning();
    await ensurePermanentAssistant();
    initWelcomeScreen();
    await initScrapers();
    ...
    // Show welcome popup after everything is loaded (non-blocking)
    if (localStorage.getItem('showWelcome')) {
        localStorage.removeItem('showWelcome');
        const langBefore = localStorage.getItem('language') || '';
        await doOnboarding(user_avatar);
        const langAfter = localStorage.getItem('language') || '';
...
export async function printCharacters(fullRefresh = false) {
    ...
    const sizeChangerOptions = [10, 25];
...
export function getEntitiesList({ doFilter = false, doSort = true } = {}) {
    ...
    if (assistantAvatar) {
        entities = entities.filter(entity => !(entity.type === 'character' && entity.item.avatar === assistantAvatar));
    }
...
export async function processCommands(message) {
    // BetterTavern: slash commands disabled for demo
    return false;
}
...
export async function duplicateCharacterByAvatar(avatarUrl) {
    if (!avatarUrl) {
        return '';
    }
    ...
    const data = await response.json();
    if (data?.path) {
        await eventSource.emit(event_types.CHARACTER_DUPLICATED, { oldAvatar: avatarUrl, newAvatar: data.path });
        await getCharacters();
        return data.path;
    }
    await getCharacters();
...
export function buildAvatarList(block, entities, { templateId = 'inline_avatar_template', empty = true, interactable = false, highlightFavs = true } = {}) {
    ...
    for (const entity of entities) {
        ...
        let avatarTitle = `[Character] ${entity.item.name}\nFile: ${entity.item.avatar}`;
        if (entity.type === 'character' && entity.item.avatar === getPermanentAssistantAvatar()) {
            avatarTemplate.addClass('assistant-avatar');
            avatarTemplate.attr('data-assistant', 'true');
            avatarTitle = `[Yumina Assistant]\n${avatarTitle}`;
        }
        avatarTemplate.attr('title', avatarTitle);
        if (highlightFavs) {
...
export async function openCharacterChat(file_name) {
    await waitUntilCondition(() => !isChatSaving && !isChatLoading, debounce_timeout.extended, 10);
    isChatLoading = true;
    try {
        await clearChat();
...
async function doOnboarding(avatarId) {
    ...
    let userName = await callGenericPopup(template, POPUP_TYPE.INPUT, currentUser?.name || name1, { wider: true, cancelButton: false, okButton: '开始旅程' });
...
export async function getSettings() {
    ...
    if (data.result != 'file not find' && data.settings) {
        ...
        if (firstRun) {
            // Remember language before onboarding so we can detect if user changed it
            const langBefore = localStorage.getItem('language') || '';
            ...
            // Force immediate save so firstRun:false persists to disk
            // (must set settingsReady first since saveSettings checks it)
            settingsReady = true;
            await saveSettings();

            // If user selected a different language during onboarding, reload now
            // (safe because firstRun:false is already saved to disk)
            const langAfter = localStorage.getItem('language') || '';
            if (langAfter !== langBefore) {
                location.reload();
...
async function messageEditDone(div) {
    ...
    // Save the edited message ID before it's cleared
    ...
    // Auto-generate if user edited their last message and AI hasn't responded yet
    // This allows users to quickly edit and get a new AI response without manually pressing regenerate
    if (editedMesId === chat.length - 1 && chat[editedMesId]?.is_user) {
...
/**
 * Restarts the current chat by reverting to the first message.
 * Keeps the same chat file, just clears all messages except the greeting.
 */
export async function restartChat() {
    ...
    if (selected_group) {
        // For groups, reload the chat which will show the first message
        await getGroupChat(selected_group, getCurrentChatId());
    } else {
        // For individual chats, add the first message (greeting)
        name2 = characters[this_chid].name;
        const message = getFirstMessage();
        if (message.mes) {
            chat.push(message);
...
jQuery(async function () {
    ...
    // [BetterTavern] LEGACY CODE - Commented out on 2026-02-05
    // This handler originally opened a character directly into chat when clicked.
    // BetterTavern replaces this with a profile card flow:
    //   1. User clicks character → Profile card shows (via bettertavern-character-profile.js)
    //   2. User clicks "Open" on profile card → selectCharacterById() is called
    // The interceptor in bettertavern-character-profile.js uses capture phase to
    // block this handler from ever receiving the click event.
    // To restore original SillyTavern behavior: uncomment this and disable the interceptor.
    //
    ...
    $('#options [id]').on('click', async function (event, customData) {
        ...
        if (id == 'option_select_chat') {
            ...
            if ((selected_group || this_chid !== undefined) && !is_send_press) {
                const result = await Popup.show.confirm(t`Restart chat?`, t`This will revert the conversation to the first message.`);
                if (!result) {
                    return;
                }
    ...
    $(document).on('click', '.mes .avatar', function (event) {
        event.preventDefault();
        event.stopPropagation();
        const messageElement = $(this).closest('.mes');
        if (messageElement.attr('is_user') == 'true' || messageElement.attr('is_system') == 'true') {
            return;
        }
        const mesId = Number(messageElement.attr('mesid'));
        const message = Number.isFinite(mesId) ? chat[mesId] : null;
        const avatarKey = message?.original_avatar || characters[this_chid]?.avatar;
        let characterIndex = avatarKey ? characters.findIndex(x => x.avatar === avatarKey) : -1;
        if (characterIndex === -1 && message?.name) {
            characterIndex = characters.findIndex(x => x.name === message.name);
        }
        if (characterIndex === -1) {
            return;
        }
        showCharacterProfile(characterIndex);
    });
